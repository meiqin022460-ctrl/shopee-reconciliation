...
def build_lazada_report(income_files, accinv_files, or_files, order_files, wallet_files,
    ...
    summary_sections = [
        ...
        ('Total Released (Net, RM) - by Order (Sheet 9 view)', round(total_released, 2), None, False),
        ('Shipping Fee (RM) - by Order (Sheet 9 view, not your money)', round(total_shipfee_settlement, 2), C_YELLOW, False),
        ('Net Revenue excl. Shipping Fee (RM) - by Order (Sheet 9 view)', round(total_net_revenue, 2), C_GREEN, False),
        ('', '', None, False),
        ('Shipping Fee (RM) - PDF ledger view (matches Sheet 6/10)', round(shipfee_pdf_scope, 2) if shipfee_pdf_scope is not None else 'N/A', C_YELLOW, False),
        ('', '', None, False),
        ('WHY TWO SHIPPING FEE NUMBERS?', '', C_HEADER, True),
        ('The "by Order" one adds up EVERY order that', '', C_GRAY, False),
        ('settled money this period (Sheet 9\'s full list).', '', C_GRAY, False),
        ('The "PDF ledger" one only counts the exact Sales+', '', C_GRAY, False),
        ('Claims lines the official PDF statement covers for', '', C_GRAY, False),
        ('this exact date range (Sheet 6/10). Different scope,', '', C_GRAY, False),
        ('so different total - both are correct for what they measure.', '', C_GRAY, False),
        ('', '', None, False),
